...
import { getCharacterUsage } from '../../src/lib/library/character-usage.js';
import type {
  LibraryClient,
  LibraryQueryBuilder,
  QueryResult,
...
describe('getCharacterUsage', () => {
  it('returns empty lists for a character with no relations', async () => {
    const { client, log } = fakeClient([
      { table: 'case_characters', data: [] },
      { table: 'location_characters', data: [] },
    ]);
    const usage = await getCharacterUsage(client, 'c1');
    expect(usage).toEqual({ locations: [], cases: [], chapters: [] });
    expect(log).toContainEqual({
      op: 'eq',
      table: 'case_characters',
      column: 'character_id',
      value: 'c1',
    });
    expect(log).toContainEqual({
  ...
  it('does not query chapter_cases/cases/chapters when the character is in no cases', async () => {
    const { client, log } = fakeClient([
      { table: 'case_characters', data: [] },
      { table: 'location_characters', data: [] },
    ]);
    await getCharacterUsage(client, 'c1');
  ...
  it('builds the cases and indirect chapters usage', async () => {
    const { client, log } = fakeClient([
      {
        table: 'case_characters',
        data: [{ case_id: 'k1', role: 'witness', required: true, min_items: 1, max_items: 3 }],
      },
      {
        table: 'cases',
  ...
  it('builds the locations usage', async () => {
    const { client } = fakeClient([
      ...
      {
        table: 'location_characters',
        data: [{ location_id: 'l1', role: 'owner', availability: false }],
      },
      {
        table: 'locations',
        data: [{ id: 'l1', name: 'Customs Hall', type: 'interior' }],
      },
    ]);
    const usage = await getCharacterUsage(client, 'c1');
    expect(usage.locations).toEqual([
      { id: 'l1', name: 'Customs Hall', type: 'interior', role: 'owner', availability: false },
    ]);
  });
  ...
  it('falls back to untitled when the referenced case row is missing', async () => {
    const { client } = fakeClient([
      {
        table: 'case_characters',
        data: [{ case_id: 'missing', role: null, required: false, min_items: 0, max_items: 0 }],
      },
      { table: 'cases', data: [] },
      { table: 'location_characters', data: [] },
    ]);
  ...
  it('throws a Database error on relation read failure', async () => {
    const { client } = fakeClient([
      { table: 'case_characters', error: { message: 'permission denied for table' } },
    ]);
    await expect(getCharacterUsage(client, 'c1')).rejects.toEqual({
      kind: 'Database',
