...
import { getDocumentUsage } from '../../src/lib/library/document-usage.js';
import type {
  LibraryClient,
  LibraryQueryBuilder,
  QueryResult,
...
describe('getDocumentUsage', () => {
  it('returns empty lists for a document with no relations', async () => {
    const { client, log } = fakeClient([
      { table: 'case_documents', data: [] },
      { table: 'location_documents', data: [] },
    ]);
    const usage = await getDocumentUsage(client, 'd1');
  ...
  it('does not query chapter_cases/cases/chapters when the document is in no cases', async () => {
    const { client, log } = fakeClient([
      { table: 'case_documents', data: [] },
      { table: 'location_documents', data: [] },
    ]);
    await getDocumentUsage(client, 'd1');
  ...
  it('builds the locations usage', async () => {
    ...
    const usage = await getDocumentUsage(client, 'd1');
    expect(usage.locations).toEqual([
      { id: 'l1', name: 'Customs Hall', type: 'interior', role: 'fake', availability: false },
    ]);
  });
  ...
  it('throws a Database error on relation read failure', async () => {
    const { client } = fakeClient([
      { table: 'case_documents', error: { message: 'permission denied for table' } },
    ]);
    await expect(getDocumentUsage(client, 'd1')).rejects.toEqual({
      kind: 'Database',
